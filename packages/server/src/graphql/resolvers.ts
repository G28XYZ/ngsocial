...
import { Post as PostEntity, Comment as CommentEntity, Like as LikeEntity, User as UserEntity } from '../entity';
...
const resolvers: Resolvers = {
	...
	Mutation: {
		register: async (_, args, { orm }) => {
			...
			const savedUser = await orm.userRepository.save(user).catch((error: unknown) => {
			...
			const token = jsonwebtoken.sign({ id: savedUser.id, email: user.email }, process.env.JWT_SECRET as string, { expiresIn: '1y' });
		...
		signIn: async (_, args, { orm }) => {
			...
			const token = jsonwebtoken.sign({ id: user.id, email: user.email }, process.env.JWT_SECRET as string, { expiresIn: '1d' });
		...
		post: async (_, args, { orm, authUser }: Context) => {
			const post = orm.postRepository.create({
				text: args.text,
				image: args.image,
				author: (await orm.userRepository.findOne({ where: { id: authUser?.id } })) as UserEntity,
		...
		comment: async (_, args, { orm, authUser }: Context) => {
			const comment = orm.commentRepository.create({
				...
				post: (await orm.postRepository.findOne({ where: { id: +args?.postId } })) as PostEntity,
				author: (await orm.userRepository.findOne({ where: { id: authUser?.id } })) as UserEntity,
			} as CommentEntity);
			...
			savedComment.post = (await orm.postRepository.findOne({ where: { id: +args.postId } })) as PostEntity;
		...
		like: async (_, args, { orm, authUser }: Context) => {
			const like = orm.likeRepository.create({
				user: await orm.userRepository.findOne({ where: { id: authUser?.id } }),
				post: await orm.postRepository.findOne({ where: { id: +args.postId } }),
			} as LikeEntity);
			const savedLike = await orm.likeRepository.save(like);
			await orm.postRepository.update(args.postId, {
				likesCount: savedLike.post.likesCount + 1,
			});
			savedLike.post = (await orm.postRepository.findOne({ where: { id: +args.postId } })) as PostEntity;
		...
		removeLike: async (_, args, { orm, authUser }: Context) => {
			const like = await orm.likeRepository
				.createQueryBuilder('like')
				.innerJoinAndSelect('like.user', 'user')
				.innerJoinAndSelect('like.post', 'post')
		...
		removePost: async (_, args, { orm }: Context) => {
			const post = await orm.postRepository.findOne({ relations: ['author'] });
			if (!post) {
		...
		setUserPhoto: async (_, args, { orm, authUser }: Context) => {
			...
			if (fileUrl != '') {
				const updateResult: UpdateResult = await orm.userRepository.update({ id: authUser?.id }, { image: fileUrl });
				if (updateResult.affected && updateResult.affected <= 0) {
					throw new ApolloError('Setting user photo failed', 'SET_USER_PHOTO_FAILED');
				}
			}
			return (await orm.userRepository.findOne({ where: { id: authUser?.id } })) as unknown as User;
		},
		setUserCover: async (_, args, { orm, authUser }: Context) => {
			...
			if (fileUrl != '') {
				const updateResult: UpdateResult = await orm.userRepository.update({ id: authUser?.id }, { coverImage: fileUrl });
				if (updateResult.affected && updateResult.affected <= 0) {
					throw new ApolloError('Setting user cover failed', 'SET_USER_COVER_FAILED');
				}
			}
		...
		setUserBio: async (_, args, { orm, authUser }: Context) => {
			const updateResult: UpdateResult = await orm.userRepository.update({ id: authUser?.id }, { bio: args.bio });
			if (updateResult.affected && updateResult.affected == 0) {
