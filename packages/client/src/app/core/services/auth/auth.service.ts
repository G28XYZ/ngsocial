...
export class AuthService {
	...
	get authState(): Observable<AuthState> {
		return this.apollo.watchQuery<{ authState: AuthState }>({ query: GET_AUTH_STATE }).valueChanges.pipe(
			map(
				(
					qr: ApolloQueryResult<{
						authState: AuthState;
					}>
				) => qr.data.authState
			)
	...
	register(fullName: string, username: string, email: string, password: string): Observable<RegisterResponse | null | undefined> {
