...
export class ProfileComponent extends BaseComponent {
	...
	fetchMore!: () => void;
	constructor(private route: ActivatedRoute, private profileService: ProfileService, public override injector: Injector) {
		super(injector);
	}
	...
	private setIsAuthUserProfile() {
		this.isAuthUserProfile = !!(this.authUser && this.authUser.id == this.profileUser?.id);
	}
	setBio(): void {
	...
	setUserCover(file: File): void {
		this.profileService
			...
			.subscribe({
				next: (userResult) => {
					if (userResult) this.handleSuccess(userResult, 'Your profile cover is successfully updated.');
				},
				error: (err) => this.handleErrors(err),
			});
	}
	setUserPhoto(file: File): void {
		this.profileService
			...
			.subscribe({
				next: (userResult) => {
					this.handleSuccess(userResult, 'Your profile photo is successfully updated.');
				},
				error: (err) => this.handleErrors(err),
			});
	}
